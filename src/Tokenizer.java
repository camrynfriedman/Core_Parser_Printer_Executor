...
/**
 * tokenizer.Tokenizer class for tokenizing an input file into a stream of
 * tokens
 * 
 * @author Maggie Feng
 */
public class Tokenizer {
	 ...
	 * @return 1 if newToken is a valid keyword, 0 if newToken is invalid token, or
	 *         -1 if cannot tell if newToken is valid or invalid yet
	 */
	private int checkKeyword(String type, String newToken, int i) {
		...
		if (newToken.equals(type)) {
			...
			if (nextIdx < this.currentLine.length() && (WHITESPACE.indexOf(this.currentLine.charAt(nextIdx)) >= 0
					|| NON_REPEATING_SYMBOLS.indexOf(this.currentLine.charAt(nextIdx)) >= 0
			...
			if (nextIdx < this.currentLine.length() && nextNextIdx < this.currentLine.length()) {
	...
	/**
	 * Checks if the there exists whitespace or special symbols in the next two
	 * indices after the current index, i
	 * 
	 * @param i the current index
	 * @return true if there is whitespace or special symbols following the current
	...
	private boolean checkWhiteSpace(int i) {
		...
		if (nextIdx < this.currentLine.length() && (WHITESPACE.indexOf(this.currentLine.charAt(nextIdx)) >= 0
			...
			return nextIdx >= this.currentLine.length();
	 ...
	 * @return int > 0 if the token + c is valid token. Otherwise, 0 if token + c is
	 *         invalid
	 */
	...
	/**
	 * Reads the next line in the input buffer and generates a list of tokens from
	 * that line
	 */
	...
	public void skipToken() {
		/*
		 * Increment the current index to point to a new token in the current line if
		 * possible
		 */
	...
	/**
	 * Returns the value of an identifier token or null if token is not an
	 * identifier
	 * 
	...
	private boolean isIdentifier(String token) {
		if (DIGITS.indexOf(token.charAt(0)) >= 0 || UPPERCASE.indexOf(token.charAt(0)) < 0) {
			return false;
		}
		for (char c : token.toCharArray()) {
	...
	/*
	 * Imported from Maggie Feng's Main.java
	 */
	public static void main(String[] args) {
		if (args.length != 1) {
			System.err.println("Invalid Arguments: Expected 'java tokenizer.Tokenizer <fileName>' ");
			return;
		}
		String fileName = args[0];
		Tokenizer tokenizer;

		// Try to initialize tokenizer.Tokenizer class
		try {
